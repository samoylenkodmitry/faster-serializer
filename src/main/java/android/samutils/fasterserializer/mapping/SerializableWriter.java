...
public class SerializableWriter extends BaseValueWriter<Parcel> {
	...
	private void startWriteField(final String fieldName) {
		Assert.assertFalse(TextUtils.isEmpty(fieldName));
		Assert.assertFalse(mFields.contains(fieldName));
		Assert.assertEquals(mFieldSizePosition, - 1);
		Assert.assertEquals(mFieldDataPosition, - 1);
